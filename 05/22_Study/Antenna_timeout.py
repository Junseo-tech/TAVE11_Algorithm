...
출력조건
- 첫째 줄에 안테나를 설치할 위치의 값을 출력합니다. 단, 안테나를 설치할 수 있는 위치 값으로 여러개의 값이 
...
for start in range(len(distance)):
    ...
    for i in range(len(distance)):
        sub = distance[start] - distance[i]
        if sub < 0 :
            sub = -(sub)
        sum += sub
    answers.append([distance[start], sum])
...
for i in range(final):
    if answers[final-(i+1)][1] == answers[final-(i+2)][1]:
        if answers[final-(i+1)][0] < answers[final-(i+2)][0]:
            print(answers[final-(i+1)][0])
        else:
            print(answers[final-(i+2)][0]) 
